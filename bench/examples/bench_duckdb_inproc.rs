//! In-process DuckDB benchmark: 65 queries × 3 runs, JSON output.
//!
//! Uses the `duckdb` crate (C API in-process) — NO CLI startup overhead.
//!
//! Loads:
//!   - ClickBench `hits` table from `/tmp/hits_1m.parquet`
//!   - All 8 TPC-H SF=1 tables from `/root/tpch-duckdb/tpch_sf1.duckdb`
//!
//! Then runs 43 ClickBench queries (read verbatim from
//! `/root/clickbench_queries.txt`) + 22 canonical TPC-H queries, 3 times each
//! after a single warm-up pass, and writes the results to
//! `/root/results/duckdb_inproc.json`.
//!
//! Run with:
//!   cargo run --release --example bench_duckdb_inproc
...
/// Canonical TPC-H SF=1 queries Q1–Q22 (standard, unmodified).
...
const HITS_PARQUET: &str = "/tmp/hits_1m.parquet";
const TPCH_DUCKDB: &str = "/root/tpch-duckdb/tpch_sf1.duckdb";
const CLICKBENCH_QUERIES_FILE: &str = "/root/clickbench_queries.txt";
const RESULTS_DIR: &str = "/root/results";
const JSON_OUT: &str = "/root/results/duckdb_inproc.json";
const LOG_OUT: &str = "/root/results/duckdb_inproc.run.log";
...
fn run_query(conn: &Connection, sql: &str) -> Result<usize, String> {
    let mut stmt = conn.prepare(sql).map_err(|e| e.to_string())?;
    let mut rows_iter = stmt.query([]).map_err(|e| e.to_string())?;
    let mut count = 0usize;
    // Rows::next returns Result<Option<&Row>, Error>
    while let Some(_row) = rows_iter.next().map_err(|e| e.to_string())? {
        count += 1;
    }
    Ok(count)
}
...
fn main() {
    println!("=== DuckDB in-process benchmark ===");
    println!("Loading ClickBench queries from {}", CLICKBENCH_QUERIES_FILE);
    ...
    // 1. Open in-process DuckDB connection (in-memory)
    println!("\nOpening in-memory DuckDB connection...");
    let conn = Connection::open_in_memory().expect("open in-memory duckdb");

    // Bump memory limit so SF=1 joins have plenty of room
    let _ = conn.execute_batch("SET memory_limit='28GB';");
    let _ = conn.execute_batch("SET threads=8;");

    // 2. Load ClickBench data
    println!("\nLoading ClickBench hits table from {} ...", HITS_PARQUET);
    assert!(Path::new(HITS_PARQUET).exists(), "hits parquet missing");
    let t0 = Instant::now();
    ...
    let clickbench_load_ms = t0.elapsed().as_secs_f64() * 1000.0;
    println!("  ClickBench load: {:.1} ms", clickbench_load_ms);

    // 3. Load TPC-H data: attach tpch_sf1.duckdb and copy the 8 tables in
    ...
    for tbl in &tpch_tables {
        ...
        let n: i64 = conn
            ...
            .unwrap_or(-1);
        println!("    {:<10} {:>10} rows", tbl, n);
    }
    let _ = conn.execute_batch("DETACH tpch;");
    let tpch_load_ms = t0.elapsed().as_secs_f64() * 1000.0;
    println!("  TPC-H load: {:.1} ms", tpch_load_ms);

    // 4. Assemble full query list: (id, suite, sql)
    let mut queries: Vec<(String, String, String)> = Vec::with_capacity(65);
    for (i, sql) in cb_sqls.iter().enumerate() {
        queries.push((format!("Q{}", i + 1), "clickbench".to_string(), sql.clone()));
    }
    for (id, sql) in TPCH_QUERIES {
        queries.push((id.to_string(), "tpch".to_string(), sql.to_string()));
    }
    assert_eq!(queries.len(), 65, "expected 65 total queries");
    println!("\nTotal queries: {} (43 clickbench + 22 tpch)", queries.len());

    // 5. Warm-up: run each query once, discard results (ignore errors here)
    ...
    // DuckDB version (runtime libduckdb version). `version()` returns e.g. "v1.5.5".
    ...
    let output = json!({
        ...
        "clickbench_load_ms": clickbench_load_ms,
        "tpch_load_ms": tpch_load_ms,
        "queries": results,
    ...
    log.push_str("=== DuckDB in-process benchmark run log ===\n");
    log.push_str(&format!("version: {}\n", duckdb_version));
    log.push_str(&format!("clickbench_load_ms: {:.2}\n", clickbench_load_ms));
    log.push_str(&format!("tpch_load_ms: {:.2}\n", tpch_load_ms));
    ...
    // 8. Summary to stdout
    println!("\n=== SUMMARY ===");
    println!("Engine: DuckDB v{} (in-process, duckdb-rs crate)", duckdb_version);
    println!("ClickBench load: {:.1} ms", clickbench_load_ms);
    println!("TPC-H load:      {:.1} ms", tpch_load_ms);
